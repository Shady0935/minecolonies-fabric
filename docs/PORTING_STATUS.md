...
- [x] Real client-to-server split envelopes reach colony handlers on the
  ...
  hiring mode are covered by the latest 52-test GameTest run
...
- [x] Projectile impacts now dispatch the cancellable `ProjectileImpactEvent`
  ...
  is covered by the latest 52-test Fabric GameTest run
...
- [x] Latest 52-test Fabric GameTest run passes with clean saves for all three
  dimensions; its 51-test core batch covers event-dispatch
  ...
  `logs/minecolonies-gametest-c2s-hiring-modes.log`
