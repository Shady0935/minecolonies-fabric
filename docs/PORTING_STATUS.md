...
- [x] Real client-to-server split envelopes reach colony handlers on the
  ...
  foundation, direct Town Hall placement, Builder work-order
  creation/selection and University research actions are covered by the
  latest 32-test GameTest run
...
- [x] The Forge-shaped Fabric event bus uses copy-on-write listener snapshots,
  ...
  colony-scoped registration/removal cannot race event dispatch; the 31-test
...
- [x] Projectile impacts now dispatch the cancellable `ProjectileImpactEvent`
  ...
  is covered by the latest 32-test Fabric GameTest run
...
- [x] Server-side builder work-order creation resolves the Colonial Town Hall
  ...
  Builder AI, construction placement and resource logistics remain open
...
- [x] Network transport smoke coverage for login/server-to-client packets plus
  real Town Hall rename, colony foundation, direct Town Hall placement,
  Builder work-order creation/selection and University research
...
- [x] Latest 32-test Fabric GameTest run passes with clean saves for all three
  dimensions; its 31-test core batch covers event-dispatch
  ...
  colony foundation, direct Town Hall placement, Builder work-order
  creation/selection and University research envelopes, all custom entity NBT
  reload contracts and populated-tavern conversion; evidence is in
  `logs/minecolonies-gametest-c2s-direct-place.log`
