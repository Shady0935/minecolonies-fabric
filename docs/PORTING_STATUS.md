...
- [x] Real client-to-server split envelopes reach colony handlers on the
  ...
  priority, Farmer field assignment, building settings and building reactivation
  are covered by the latest 42-test GameTest run
...
- [x] Projectile impacts now dispatch the cancellable `ProjectileImpactEvent`
  ...
  is covered by the latest 42-test Fabric GameTest run
...
- [x] Latest 42-test Fabric GameTest run passes with clean saves for all three
  dimensions; its 41-test core batch covers event-dispatch
  ...
  `logs/minecolonies-gametest-c2s-reactivate.log`
