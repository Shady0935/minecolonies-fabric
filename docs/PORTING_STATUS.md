...
- [x] Real client-to-server split envelopes reach colony handlers on the
  ...
  priority are covered by the latest 39-test GameTest run
...
- [x] Projectile impacts now dispatch the cancellable `ProjectileImpactEvent`
  ...
  is covered by the latest 39-test Fabric GameTest run
...
- [x] Latest 39-test Fabric GameTest run passes with clean saves for all three
  dimensions; its 38-test core batch covers event-dispatch
  ...
  FarmField registration/seed/radius updates,
  all custom entity NBT reload contracts and populated-tavern
  conversion; evidence is in
  `logs/minecolonies-gametest-c2s-delivery-priority-isolated.log`
