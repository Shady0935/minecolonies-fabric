...
- [x] Real client-to-server split envelopes reach colony handlers on the
  ...
  priority and Farmer field assignment are covered by the latest 40-test
  GameTest run
...
- [x] Projectile impacts now dispatch the cancellable `ProjectileImpactEvent`
  ...
  is covered by the latest 40-test Fabric GameTest run
...
- [x] Latest 40-test Fabric GameTest run passes with clean saves for all three
  dimensions; its 39-test core batch covers event-dispatch
  ...
  FarmField registration/seed/radius updates and Farmer field assignment
  mode/assign/free updates,
  all custom entity NBT reload contracts and populated-tavern
  conversion; evidence is in
  `logs/minecolonies-gametest-c2s-farmer-fields.log`
