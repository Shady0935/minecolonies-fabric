...
- [x] Real client-to-server split envelopes reach colony handlers on the
  ...
  actions, colony allocation toggles and colony-flag updates are covered by
  the latest 38-test GameTest run
...
- [x] Projectile impacts now dispatch the cancellable `ProjectileImpactEvent`
  ...
  is covered by the latest 38-test Fabric GameTest run
...
- [x] Latest 38-test Fabric GameTest run passes with clean saves for all three
  dimensions; its 37-test core batch covers event-dispatch
  ...
  colony allocation and flag updates, FarmField registration/seed/radius updates,
  all custom entity NBT reload contracts and populated-tavern
  conversion; evidence is in
  `logs/minecolonies-gametest-c2s-colony-management.log`
