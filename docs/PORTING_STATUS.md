...
- [x] Real client-to-server split envelopes reach colony handlers on the
  ...
  marking, move-in policy, citizen pause/resume and Farmer hire/fire are covered
  by the latest 48-test GameTest run
...
- [x] Projectile impacts now dispatch the cancellable `ProjectileImpactEvent`
  ...
  is covered by the latest 48-test Fabric GameTest run
...
- [x] Latest 48-test Fabric GameTest run passes with clean saves for all three
  dimensions; its 47-test core batch covers event-dispatch
  ...
  `logs/minecolonies-gametest-c2s-hire-fire.log`
