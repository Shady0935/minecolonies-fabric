...
| Fabric 1.20.1 package build | `AUTOMATED_PASS` | `project/minecolonies/gradlew.bat build --no-daemon` passed after the mob-conversion bridge; evidence is in `logs/minecolonies-build-conversion.log`. The final JAR contains all 1,258 JSON resources and zero datagen-cache entries. |
...
| Colony protection callbacks | `AUTOMATED_PASS` | `logs/minecolonies-gametest-conversion-2.log` drives Fabric's `UseBlockCallback` through the Town Hall permission handler: an outsider is denied and the colony owner is accepted in an isolated fixture. |
| Mob conversion callback | `AUTOMATED_PASS` | `logs/minecolonies-gametest-conversion-2.log` dispatches Fabric's `MOB_CONVERSION` callback into the retained `LivingConversionEvent.Pre` bridge and confirms an unrelated conversion candidate is not discarded. |
...
| Town Hall GUI/colony creation | `AUTOMATED_PASS` / `MANUAL_VALIDATION_PENDING` | Server-side creation, ownership lookup, Town Hall registration, protection callback and NBT round-trip pass in `logs/minecolonies-gametest-conversion-2.log`; GUI/client interaction remains pending. |
...
| Automated tests | `AUTOMATED_PASS` | The Fabric GameTest batch in `logs/minecolonies-gametest-conversion-2.log` reports `All 7 required tests passed`. |
