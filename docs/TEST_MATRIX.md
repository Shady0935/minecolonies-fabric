...
| Fabric 1.20.1 package build | `AUTOMATED_PASS` | `project/minecolonies/gradlew.bat build --no-daemon` passed after the citizen fixture; evidence is in `logs/minecolonies-build-citizen.log`. The final JAR contains all 1,258 JSON resources and zero datagen-cache entries. |
...
| Automated tests | `AUTOMATED_PASS` | The Fabric GameTest batch in `logs/minecolonies-gametest-citizen.log` reports `All 4 required tests passed`. |
