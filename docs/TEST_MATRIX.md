...
| Fabric 1.20.1 package build | `AUTOMATED_PASS` | `project/minecolonies/gradlew.bat build --no-daemon` passed after the spear renderer lifecycle fix; evidence is in `logs/minecolonies-build-spear-renderer-fixed.log`. The final JAR contains all 1,258 JSON resources and zero datagen-cache entries. |
...
| Client bootstrap | `AUTOMATED_PASS` | `logs/minecolonies-client-spear-renderer-fixed.log` reaches resource reload, OpenAL and all texture atlases without the earlier spear-renderer initialization crash. |
...
| Automated tests | `AUTOMATED_PASS` | The latest Fabric GameTest batch in `logs/minecolonies-gametest-spear-renderer.log` reports `All 9 required tests passed`, followed by clean world and dimension saves. |
