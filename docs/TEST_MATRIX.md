...
| Fabric 1.20.1 package build | `AUTOMATED_PASS` | `project/minecolonies/gradlew.bat build --no-daemon` passed after the protection fixture; evidence is in `logs/minecolonies-build-protection.log`. The final JAR contains all 1,258 JSON resources and zero datagen-cache entries. |
...
| Colony protection callbacks | `AUTOMATED_PASS` | `logs/minecolonies-gametest-protection.log` drives Fabric's `UseBlockCallback` through the Town Hall permission handler: an outsider is denied and the colony owner is accepted. |
...
| Town Hall GUI/colony creation | `AUTOMATED_PASS` / `MANUAL_VALIDATION_PENDING` | Server-side creation, ownership lookup, Town Hall registration, protection callback and NBT round-trip pass in `logs/minecolonies-gametest-protection.log`; GUI/client interaction remains pending. |
...
| Automated tests | `AUTOMATED_PASS` | The Fabric GameTest batch in `logs/minecolonies-gametest-protection.log` reports `All 6 required tests passed`. |
