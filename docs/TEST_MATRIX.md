...
| Fabric 1.20.1 package build | `AUTOMATED_PASS` | `project/minecolonies/gradlew.bat build --no-daemon`; the lifecycle-integrated verification is recorded in `logs/minecolonies-build-lifecycle.log`. The final JAR contains all 1,258 JSON resources and zero datagen-cache entries. |
...
| Client bootstrap | `AUTOMATED_PASS` | `logs/minecolonies-runclient-lifecycle.log` reaches OpenAL and all texture atlases without a crash. |
...
| Serialization and saved colony data | `MANUAL_VALIDATION_PENDING` | Server persistence was tested for the world lifecycle; colony-specific save/load and DataFixer migration remain untested. An isolated citizen with `colony=0` is deliberately removed by the upstream colony handler, so that smoke test is not a persistence claim. |
