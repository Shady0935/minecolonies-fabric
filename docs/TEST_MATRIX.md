...
| Fabric 1.20.1 package build | `AUTOMATED_PASS` | `project/minecolonies/gradlew.bat build --no-daemon`; the network-codec verification is recorded in `logs/minecolonies-build-network-codec.log`. The final JAR contains all 1,258 JSON resources and zero datagen-cache entries. |
...
| Automated tests | `AUTOMATED_PASS` | The Fabric GameTest batch in `logs/minecolonies-gametest-network-codec.log` reports `All 3 required tests passed`. |
