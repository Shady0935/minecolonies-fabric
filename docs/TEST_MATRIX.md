...
| Fabric 1.20.1 package build | `AUTOMATED_PASS` | `project/minecolonies/gradlew.bat build --no-daemon` passed after the bow-hook bridge; evidence is in `logs/minecolonies-build-arrow-hooks.log`. The final JAR contains all 1,258 JSON resources and zero datagen-cache entries. |
...
| Automated tests | `AUTOMATED_PASS` | The Fabric GameTest batch in `logs/minecolonies-gametest-arrow-hooks.log` reports `All 9 required tests passed`. |
