...
`logs/minecolonies-gametest-tavern-10.log`. The current 52-test run also
...
the building; the latest 52-test run therefore resolves the structure without
...
Quarry hiring-mode transition. The
...
`logs/minecolonies-gametest-c2s-hiring-modes.log`; it reports
`All 52 required tests passed` (51 core tests plus one entity batch test).
