...
The runtime resource source set also includes the tracked official 1.20.1
generated snapshot at `project/minecolonies/src/datagen/generated/minecolonies`.
This keeps content that is not yet emitted by a native Fabric provider
(advancements, quest translations and entity icons) available in-game. Files
regenerated by Fabric under `src/main/generated/resources` take precedence on
duplicate paths; the snapshot remains a parity bridge for the still-excluded
providers, not a replacement for their implementation.
...
The verified run registered 34 providers and generated 895 JSON resources.
...
the packaged resources. The packaged JAR additionally contains the tracked
upstream parity snapshot, yielding 1,453 JSON resources in total and 2,599
generated citizen/raider icon PNGs. Its paths are rooted directly at
`assets/` and `data/`, and the client resource reload was re-run after adding
it.
...
Five formerly Forge-shaped providers now run natively on Fabric: block tags,
damage tags, damage types, entity type tags and item tags. The port also
generates the missing `c:` conventional block/item tags that Fabric API
0.92.2 does not ship, so worker recipes and item filters do not resolve to
missing tags at runtime. Three providers remain excluded explicitly from
`runDatagen`: default advancements, entity icons and quest translations.
Their validated upstream outputs are packaged through the parity snapshot
above, but the provider graph itself still needs native Fabric implementations
for clean regeneration from source. This is a tracked implementation gap, not
a silent no-op.
