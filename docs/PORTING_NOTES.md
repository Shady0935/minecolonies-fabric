...
The Cook companion fixture registers a Colonial level-one Cook, places and
registers a vanilla Furnace, assigns a live `JobCook` and confirms that
`EntityAIWorkCook` is created. It seeds a beef cooking request and deterministic
dried-kelp-block fuel, then verifies raw-food/fuel consumption, vanilla furnace
