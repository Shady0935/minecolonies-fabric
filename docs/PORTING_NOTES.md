...
`logs/minecolonies-gametest-tavern-10.log`. The current 34-test run also
...
the building; the latest 34-test run therefore resolves the structure without
...
`DecorationBuildRequestMessage` and `HutRenameMessage`,
wraps each in the
...
rotation/mirror preservation, plus building custom-name mutation. The research,
...
`logs/minecolonies-gametest-c2s-hut-rename-fixed.log`; it reports
`All 34 required tests passed` (33 core tests plus one entity batch test).
