...
and an inherited handler, and the latest 27-test core batch exercises this under
...
`logs/minecolonies-gametest-tavern-10.log`. The current 28-test run also
...
the building; the latest 28-test run therefore resolves the structure without
...
retained `ResearchListener`. The latest 27-test core GameTest batch resolves all four
...
serializes `TownHallRenameMessage`, wraps it in the real split envelope, sends
it through `SimpleChannel`'s server dispatch boundary and verifies the deferred
server-executor action, owner permission and colony-name mutation. Evidence is
in `logs/minecolonies-gametest-c2s-envelope.log`; the complete run reports
