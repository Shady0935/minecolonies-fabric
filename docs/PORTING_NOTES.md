...
This keeps content that is not yet emitted by a native Fabric provider (tags,
advancements, damage data, quest files and entity icons) available in-game.
Files regenerated by Fabric under `src/main/generated/resources` take
precedence on duplicate paths; this snapshot is a parity bridge, not a claim
that its eight Forge-shaped provider implementations have already been
ported.
...
upstream parity snapshot, yielding 1,434 JSON resources in total and 2,599
...
The eight Forge-only generation providers remain excluded explicitly from
`runDatagen`: default advancements, block tags, damage tags, damage types,
entity icons, entity type tags, item tags and quest translations. Their
validated upstream outputs are packaged through the parity snapshot above,
but the provider graph itself still needs native Fabric implementations for
clean regeneration from source. This is a tracked implementation gap, not a
silent no-op.
