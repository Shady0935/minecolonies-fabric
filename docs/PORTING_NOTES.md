...
`logs/minecolonies-gametest-tavern-10.log`. The current 62-test run also
...
the building; the latest 62-test run therefore resolves the structure without
...
updates, Smeltery item-filter add/remove/reset, GiveTool inventory binding and
citizen inventory transfer are also covered. The
...
`logs/minecolonies-gametest-c2s-citizen-inventory-retry.log`; it reports
`All 62 required tests passed` (61 core tests plus one entity batch test).
