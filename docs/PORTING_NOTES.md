...
`logs/minecolonies-gametest-tavern-10.log`. The current 55-test run also
...
the building; the latest 55-test run therefore resolves the structure without
...
work-order priority and removal transitions, plus residence assignment and
unassignment, are also covered. The
...
`logs/minecolonies-gametest-c2s-assign-unassign.log`; it reports
`All 55 required tests passed` (54 core tests plus one entity batch test).
