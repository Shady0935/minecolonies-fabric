...
`logs/minecolonies-gametest-tavern-10.log`. The current 41-test run also
...
the building; the latest 41-test run therefore resolves the structure without
...
`logs/minecolonies-gametest-c2s-farmer-settings.log`; it reports
`All 41 required tests passed` (40 core tests plus one entity batch test).
