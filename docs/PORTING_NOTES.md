...
`logs/minecolonies-gametest-tavern-10.log`. The current 35-test run also
...
the building; the latest 35-test run therefore resolves the structure without
...
`ColonyStructureStyleMessage` and `ColonyTextureStyleMessage`,
wraps each in the
...
`logs/minecolonies-gametest-c2s-colony-style.log`; it reports
`All 35 required tests passed` (34 core tests plus one entity batch test).
