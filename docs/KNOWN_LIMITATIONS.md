...
- The Fabric GameTest now covers server-side Colonial Town Hall creation,
  ...
  It does not yet cover cross-warehouse or multi-destination route scheduling
  or broad logistics beyond the focused two-material Builder work-order cycle
  (two racks → two Stack/Delivery requests → Courier → Builder), larger
  ...
  placement, multi-cell harvest behavior and warehouse/courier delivery beyond
  the focused 24-block harvest → Farmer-hut deposit → pickup-request cycle, a broader
...
- Authenticated in-game interaction with BlockUI screens (including the
  ...
  cross-warehouse/multi-destination Courier scheduling and broad stock logistics, full citizen work
  cycles/scheduling,
  ...
  covered 24-block harvest, Farmer-hut deposit and pickup-request route (the direct empty-field hoeing
  ...
  blueprint with stone and cobblestone requested from separate Warehouse racks
  through separate normal Stack/Delivery requests. The assigned Courier
  autonomously navigates from each rack to the Builder through its `CitizenAI`,
  then the Builder consumes both materials. Larger canonical builds,
  cross-warehouse routes and longer
...
- The latest combined 114-test GameTest run still logs the upstream-style warning that
  ...
  `logs/minecolonies-gametest-builder-two-racks-20260923.log`. The
