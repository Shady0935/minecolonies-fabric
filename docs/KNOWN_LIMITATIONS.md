...
- A small number of non-power-of-two textures reduce their mip level, and one
  ...
  bootstrap to fail.
- The latest combined 116-test GameTest run still logs the upstream-style warning that
  ...
  `logs/minecolonies-gametest-c2s-permission-management-final-20260924.log`. The
  blueprint data should be audited before treating this warning as resolved.
- The latest clean combined 116-case suite hardens only the transient Raider combat
  fixture state before starting the real `RaiderMeleeAI`/`AttackMoveAI` path
  and records 116/116 required tests passed, including the three focused
