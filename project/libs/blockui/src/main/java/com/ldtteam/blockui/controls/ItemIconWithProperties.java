...
public class ItemIconWithProperties extends ItemIcon
{
    ...
    public void drawSelf(final BOGuiGraphics target, final double mx, final double my)
    {
        ...
        genericPropertyOverrides.forEach((key, val) -> {
            ...
            ItemProperties.registerGeneric(key, asClamped(val));
        });
        ...
        currentItemOverrides.forEach((key, val) -> {
            ...
            ItemProperties.register(item, key, asClamped(val));
        });

        super.drawSelf(target, mx, my);

        oldItemVals.forEach((key, val) -> ItemProperties.register(item, key, asClamped(val)));
        oldGenericVals.forEach((key, val) -> ItemProperties.registerGeneric(key, asClamped(val)));
    }

    private static net.minecraft.client.renderer.item.ClampedItemPropertyFunction asClamped(final ItemPropertyFunction property)
    {
        return (itemStack, level, entity, seed) -> property.call(itemStack, level, entity, seed);
    }
