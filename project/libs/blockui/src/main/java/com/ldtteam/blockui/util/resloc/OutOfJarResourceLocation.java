...
public class OutOfJarResourceLocation extends ResourceLocation
{
        ...
        @Nullable final Function<DefaultPlayerSkin, ResourceLocation> textureSelector)
    {
        ...
        // The 1.20.1 vanilla HttpTexture keeps its cache file private. Returning
        // the normal skin location preserves the supported vanilla path without
        // depending on an inaccessible Forge/implementation field.
        return skinResLoc;
    }
    ...
    public static Resource getResourceHandle(final ResourceLocation resLoc, final ResourceManager fallbackManager) throws IOException
    {
        if (resLoc instanceof final OutOfJarResourceLocation nioResLoc)
        {
            final Path metadataPath = nioResLoc.getNioPath().resolveSibling(nioResLoc.getNioPath().getFileName() + ".mcmeta");
            return fileExists(nioResLoc.withSuffix(".mcmeta"), fallbackManager) ?
                new OutOfJarResource(nioResLoc, () -> ResourceMetadata.fromJsonStream(Files.newInputStream(metadataPath))) :
