...
public class ServerStructurePackLoader
{
    ...
    /** Registers the server tick hook used for deferred pack transfers. */
    ...
    public static void onServerStarting()
    {
        ...
        final List<String> modOrigins = new ArrayList<>();
        final List<String> modList = new ArrayList<>();
        FabricLoader.getInstance().getAllMods().forEach(mod -> {
            final String modId = mod.getMetadata().getId();
            modList.add(modId);
            mod.findPath(BLUEPRINT_FOLDER).ifPresent(path -> {
                modPaths.add(path);
                modOrigins.add(modId);
            });
        });
    ...
    public static void onWorldTick(final ServerLevel level)
    {
        ...
        {
            if (loadingState == ServerLoadingState.FINISHED_LOADING && !clientSyncRequests.isEmpty())
            {
            ...
            if (!messageSendTasks.isEmpty())
            {
                ...
                final ServerPlayer player = level.getServer().getPlayerList().getPlayer(packData.player);
