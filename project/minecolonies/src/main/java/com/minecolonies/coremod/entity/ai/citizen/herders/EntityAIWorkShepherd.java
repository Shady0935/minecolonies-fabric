...
public class EntityAIWorkShepherd extends AbstractEntityAIHerder<JobShepherd, BuildingShepherd>
{
    ...
    private IAIState shearSheep()
    {
        ...
        if (worker.getMainHandItem() != null)
        {
            ...
            int enchantmentLevel = net.minecraft.world.item.enchantment.EnchantmentHelper.getItemEnchantmentLevel(Enchantments.BLOCK_FORTUNE, worker.getMainHandItem());
            ...
            if (!this.world.isClientSide)
            {
                ...
                for(int j = 0; j < qty; ++j)
                {
                    items.add(new ItemStack(woolFor(sheep.getColor())));
                }
            }

            sheep.playSound(SoundEvents.SHEEP_SHEAR, 1.0F, 1.0F);
            Network.getNetwork().sendToTrackingEntity(new LocalizedParticleEffectMessage(new ItemStack(woolFor(sheep.getColor())), sheep.getOnPos().above()), worker);
    ...
    private static net.minecraft.world.item.Item woolFor(final DyeColor color)
    {
        return switch (color)
        {
